...
public class SpringControllerImplTest {
	...
	final static String ANOTHER_OBJECT = "another";
	...
	public void update() {
		...
		EasyMock.expect(dao.update(OBJECT)).andReturn(ANOTHER_OBJECT);
		EasyMock.replay(dao);

		final String returned = controller.update(OBJECT);
		EasyMock.verify(dao);

		assert returned == ANOTHER_OBJECT;
	...
	final private static class DummyGenericController extends SpringControllerImpl<String, Integer> {
